...
public enum ResultStatus {
  ...
  UN_KNOWN(1000,"UN KNOWN"),
  EXCEL_FORMAT_EXCEPTION(1001,"EXCEL FORMAT EXCEPTION"),
  ...
  private ResultStatus instance(int errorCode){
    for(ResultStatus error: values()){
      if(errorCode == error.code){
        return error;
      }
    }
    return ResultStatus.UN_KNOWN;
  }
